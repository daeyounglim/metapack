...
#' are diagonal matrices with zero covariances. M2 indicates that \eqn{\Sigma_{tk}} are all equivalent but allowed to be full symmetric
#' positive definite. M3 is where \eqn{\Sigma_{tk}} are allowed to differ across treatments, i.e., \eqn{\Sigma_{tk}=\Sigma_t}.
...
#' Finally, M5 assumes a hierarchical model where \eqn{(\Sigma_{tk} | \Sigma)} follows an inverse-Wishart distribution with fixed
